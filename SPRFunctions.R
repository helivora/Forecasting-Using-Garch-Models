##########################################################################
##	Compute AIC and BIC of candidate models
##	the conditional mean model is fixed and given in argument arma
##	Valid values for var.model are "sGARCH", "eGARCH", "apARCH", 
##			"gjrGARCH", "TGARCH"
##
##	Valid values for dist are "norm", "snorm", "std", "sstd", 
##			"ged", "sged", "nig", "jsu" 
##	Returned object includes the AIC and BIC scores of all candidate models
##			listed for each var.model
##	Print model selected by the AIC and BIC for each var.model
##
##########################################################################

ic.fun = function(Yn, max.p = 2, max.q =2, arma = c(1,1), var.model = c("sGARCH"), dist = c("std")){
    pk = max.p; qk = max.q;  mk = length(var.model); dk = length(dist)
    ic = vector("list", length = mk)
    mean.model = list(armaOrder = c(0,1), include.mean = F)
    aic.min = c(); bic.min = c()
    for(l in 1:mk){
        garch = var.model[l]
        out = array(dim = c(pk*qk, 4, dk))       
        for(m in 1:dk){ 
            dtn = dist[m]
            ind = 0
            for(i in 1:pk){
                for(j in 1:qk){
                    ind = ind + 1; 
                    out[,,m][ind,c(1:2)] = c(i,j)

                    if(garch == "tGARCH" || garch == "TGARCH") {
                        spec = ugarchspec(variance.model = list(model = "fGARCH",       submodel = "TGARCH", garchOrder = c(i,j)), 
                            mean.model = mean.model, distribution.model = dtn)
                    }
                    else{
                        spec = ugarchspec(variance.model = list(model = garch, garchOrder = c(i,j)), mean.model = mean.model,   distribution.model = dtn)
                    }                    
                    fit = ugarchfit(data = Yn, spec=spec)
                    out[,,m][ind,c(3:4)] =  infocriteria(fit)[1:2,]
                }
            }
        }
        out = matrix(aperm(out, perm = c(1,3,2) ),ncol = 4)
        colnames(out) = c("p","q", "aic", "bic");
        aic.min = c(aic.min, which.min(out[,"aic"]))
        bic.min = c(bic.min, which.min(out[,"bic"]))
        ic[[l]] = data.frame(out, dist = rep(dist, each = pk*qk))
    }
    ret.a = data.frame(NULL)
    ret.b = data.frame(NULL)
    for(l in 1:mk){
        ret.a = rbind(ret.a, ic[[l]][aic.min[l], -4])
        ret.b = rbind(ret.b, ic[[l]][bic.min[l],-3])
    }
    rownames(ret.a) = rownames(ret.b) = var.model
